...
public partial class MainForm : Form
{
    ...
    private void TranslateControls()
    {
        ...
        State = State;
        // Removed: VersionLabel.Text = UpdateChecker.Version;
    }
    ...
    */
    ...
    /* Removed: Version and update related handlers
    ...
    */
    ...
    private async void SpeedPictureBox_Click(object sender, EventArgs e)
    {
        ...
        if (IsWaiting())
        {
            MessageBoxX.Show("请先启动连接后再测试IP和地区信息", LogLevel.WARNING, "提示");
            return;
        }
        ...
        if (server == null)
        {
            MessageBoxX.Show("请先选择一个服务器", LogLevel.WARNING, "提示");
            Enable();
            return;
        }
        ...
        // 测试延迟
        await server.PingAsync();
        var delayText = server.Delay >= 0 ? $"{server.Delay}ms" : (server.Delay == -2 ? "DNS解析失败" : "连接失败");
        ...
        try
        {
            ...
            try
            {
                ...
                ipInfo = $"\n\nIP: {ip}\n地区: {country}" + (string.IsNullOrEmpty(city) ? "" : $" {city}");
            }
            catch
            {
                // 如果ip.sb失败，尝试备用API
        ...
        var message = $"服务器: {server.Remark}\n延迟: {delayText}{ipInfo}";
        MessageBoxX.Show(message, LogLevel.INFO, "测速结果");
