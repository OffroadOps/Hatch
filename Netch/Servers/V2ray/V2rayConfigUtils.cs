...
public static class V2rayConfigUtils
{
    ...
    private static async Task<Outbound> outbound(Server server)
    {
        ...
        switch (server)
        {
            ...
            case VLESSServer vless:
            {
                ...
                {
                    userFlow = "xtls-rprx-direct";
                }
                else if (vless.TLSSecureType == "reality" || !string.IsNullOrEmpty(vless.Flow))
                {
                    userFlow = vless.Flow ?? "xtls-rprx-vision";
                }
                ...
                // Reality 和 XTLS Vision 不支持 Mux
                if (vless.TLSSecureType == "xtls" || vless.TLSSecureType == "reality" || !string.IsNullOrEmpty(vless.Flow))
                {
                    outbound.mux.enabled = false;
                    outbound.mux.concurrency = -1;
                }
    ...
    private static StreamSettings boundStreamSettings(VMessServer server)
    {
        ...
        if (server.TLSSecureType == "reality" && server is VLESSServer vless)
        {
            ...
            {
                serverName = vless.ServerName.ValueOrDefault() ?? vless.Host.SplitOrDefault()?[0] ?? server.Hostname,
                fingerprint = vless.RealityFingerprint ?? "chrome",
                publicKey = vless.RealityPublicKey ?? "",
                shortId = vless.RealityShortId ?? "",
                spiderX = vless.RealitySpiderX ?? ""
            };
        }
        else if (server.TLSSecureType != "none")
        {
            var tlsSettings = new TlsSettings
            {
                allowInsecure = Global.Settings.V2RayConfig.AllowInsecure,
