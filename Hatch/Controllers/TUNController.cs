...
using Hatch.Interops;
using Hatch.Models;
using Hatch.Models.Modes;
using Hatch.Models.Modes.TunMode;
using Hatch.Servers;
using Hatch.Utils;
using static Hatch.Interops.tun2socks;

namespace Hatch.Controllers
{
    public class TUNController : IModeController
    {
        /// <summary>等待 TUN 网卡创建的最大轮询次数</summary>
        private const int AdapterCreateMaxPollCount = 20;
        /// <summary>每次轮询间隔（毫秒），总等待 = 20 × 300 = 6秒</summary>
        private const int AdapterCreatePollIntervalMs = 300;
        /// <summary>默认 MTU 值</summary>
        private const string DefaultMTU = "1500";
        ...
        public async Task StartAsync(Socks5Server server, Mode mode)
        {
            ...
            for (var i = 0; i < AdapterCreateMaxPollCount; i++)
            {
                await Task.Delay(AdapterCreatePollIntervalMs);
                try
                {
            ...
            Dial(NameList.TYPE_ADAPMTU, DefaultMTU);
        ...
        {
            string binDriver = Path.Combine(Global.HatchDir, Constants.WintunDllFile);
