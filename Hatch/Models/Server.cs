...
public abstract class Server : ICloneable
{
    ...
    public override string ToString()
    {
        ...
        var coreTag = GetCoreDisplayName();  // 获取内核标识，如 "Xray" 或 "sing"

        return $"[{coreTag}][{Group}] {remark}";
    }
    ...
    /// 获取此服务器应该使用的内核
    /// </summary>
    /// <returns>内核类型</returns>
    public CoreType GetRequiredCore()
    {
        return CoreSelector.SelectCoreForServer(this);
    }

    /// <summary>
    /// 获取内核显示名称（用于UI显示，如 "Xray" 或 "sing"）
    /// </summary>
    public string GetCoreDisplayName()
    {
        return CoreSelector.GetCoreDisplayName(GetRequiredCore());
    }

    /// <summary>
    /// 获取内核完整名称（如 "Xray-core" 或 "sing-box"）
    /// </summary>
    public string GetCoreFullName()
    {
        return CoreSelector.GetCoreFullName(GetRequiredCore());
    }
        ...
        try
        {
            ...
            for (var i = 0; i < 3; i++)
            {
                Task<int> PingCoreAsync()
                {
                    try
                    {
                        // Hysteria2 是 UDP 协议，强制使用 ICMP Ping
                        if (Type == "Hysteria2")
                        {
                            return Utils.Utils.ICMPingAsync(destination);
                        }
