...
public static class Bandwidth
{
    ...
    {
        NetTrafficAsync().ContinueWith(t =>
        {
            if (t.IsFaulted && t.Exception != null)
                Log.Error(t.Exception, "Net traffic monitoring failed");
        });
    }

    private static async Task NetTrafficAsync()
    {
        ...
        Task.Run(() =>
            ...
            {
                if (t.IsFaulted && t.Exception != null)
                    Log.Error(t.Exception, "ETW trace session failed");
            });
