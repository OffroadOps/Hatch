...
public static class Utils
{
    public static void Open(string path)
    {
        try
        {
            ...
            {
                // 本地路径：规范化后确保不包含命令注入字符
                var fullPath = Path.GetFullPath(path);
                if (!File.Exists(fullPath) && !Directory.Exists(fullPath))
                {
                    Log.Warning("Open path does not exist: {Path}", fullPath);
                    return;
                }
    ...
    public static async Task<string> GetCityCodeAsync(string address)
    {
        ...
        try
        {
            using var databaseReader = new DatabaseReader("bin\\GeoLite2-Country.mmdb");
        ...
        {
            Log.Warning(e, "IP country lookup failed for {Address}", address);
        }
    ...
    public static void RegisterHatchStartupItem()
    {
        const string TaskName = "Hatch Startup";
        ...
        if (Global.Settings.RunAtStartup)
        {
            ...
            td.RegistrationInfo.Author = "Hatch";
            td.RegistrationInfo.Description = "Hatch run at startup.";
            td.Principal.RunLevel = TaskRunLevel.Highest;

            td.Triggers.Add(new LogonTrigger());
            td.Actions.Add(new ExecAction(Global.HatchExecutable));
            ...
            TaskService.Instance.RootFolder.RegisterTaskDefinition("Hatch Startup", td);
        }
        else
        {
            if (taskIsExists)
