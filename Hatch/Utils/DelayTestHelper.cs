...
public static class DelayTestHelper
{
    ...
    static DelayTestHelper()
    {
        ...
        Timer.Elapsed += (_, _) => PerformTestAsync().ContinueWith(t =>
        {
            if (t.IsFaulted && t.Exception != null)
                Log.Error(t.Exception, "Periodic delay test failed");
        });
    }
    ...
    private static void UpdateTick(int interval, bool performTestAtOnce = false)
    {
        ...
        if (enable)
        {
            ...
            {
                PerformTestAsync().ContinueWith(t =>
                {
                    if (t.IsFaulted && t.Exception != null)
                        Log.Error(t.Exception, "Initial delay test failed");
                });
            }
        }
    }
