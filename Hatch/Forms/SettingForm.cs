...
public partial class SettingForm : BindingForm
{
    public SettingForm()
    {
        ...
        BindTextBox<int>(Hysteria2UpMbpsTextBox,
            i => i > 0,
            i => Global.Settings.CoreConfig.SingBox.Hysteria2UpMbps = i,
            Global.Settings.CoreConfig.SingBox.Hysteria2UpMbps);

        BindTextBox<int>(Hysteria2DownMbpsTextBox,
            i => i > 0,
            i => Global.Settings.CoreConfig.SingBox.Hysteria2DownMbps = i,
            Global.Settings.CoreConfig.SingBox.Hysteria2DownMbps);

        TUICCongestionControlComboBox.Enabled = false;

        BindCheckBox(SingBoxMultiplexCheckBox,
            b => Global.Settings.CoreConfig.SingBox.UseMultiplex = b,
            Global.Settings.CoreConfig.SingBox.UseMultiplex);

        BindCheckBox(SingBoxTCPFastOpenCheckBox,
            b => Global.Settings.CoreConfig.SingBox.TCPFastOpen = b,
            Global.Settings.CoreConfig.SingBox.TCPFastOpen);
    ...
    {
        ControlButtonClickAsync().ContinueWith(t =>
        {
            if (t.IsFaulted && t.Exception != null)
                Log.Error(t.Exception, "Save settings failed");
        });
    }

    private async Task ControlButtonClickAsync()
    {
        ...
        Utils.Utils.RegisterHatchStartupItem();
