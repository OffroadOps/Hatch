...
using Hatch.Models.Modes.ProcessMode;
...
public partial class ProcessForm : BindingForm
{
    ...
    private void InitWindowsStoreAppButtons()
    {
        var handleStoreAppsButton = CreateStoreAppsButton();
        handleStoreAppsButton.Tag = HandleRuleRichTextBox;
        handleStoreAppsButton.Click += StoreAppsButton_Click;

        var bypassStoreAppsButton = CreateStoreAppsButton();
        bypassStoreAppsButton.Tag = BypassRuleRichTextBox;
        bypassStoreAppsButton.Click += StoreAppsButton_Click;

        HandleHelperFlowLayoutPanel.Width = 372;
        BypassFlowLayoutPanel.Width = 372;
        HandleHelperFlowLayoutPanel.Controls.Add(handleStoreAppsButton);
        BypassFlowLayoutPanel.Controls.Add(bypassStoreAppsButton);
    }

    private static Button CreateStoreAppsButton() => new()
    {
        Text = "Windows 应用",
        AutoSize = true,
        Height = 23,
    ...
    private async void StoreAppsButton_Click(object? sender, EventArgs e)
    {
        if (sender is Button { Tag: RichTextBox ruleRichTextBox })
            await AddWindowsStoreAppRulesAsync(ruleRichTextBox);
    }
    ...
    private async Task AddWindowsStoreAppRulesAsync(RichTextBox ruleRichTextBox)
    {
        try
        {
            Cursor = Cursors.WaitCursor;
            var apps = await WindowsStoreAppHelper.GetInstalledAppsAsync();
            Cursor = Cursors.Default;

            if (!apps.Any())
            {
                MessageBoxX.Show(i18N.Translate("No Windows apps found"));
                return;
            }

            using var form = new WindowsStoreAppSelectForm(apps);
            if (form.ShowDialog(this) != DialogResult.OK)
                return;

            var rules = form.SelectedApps
            ...
            AppendText(ruleRichTextBox, string.Join(Constants.EOF, rules));
        }
        catch (Exception e)
        {
            Log.Warning(e, "Enumerate Windows apps failed");
            MessageBoxX.Show(i18N.Translate("Enumerate Windows apps failed") + Constants.EOF + e.Message, LogLevel.WARNING);
        }
        finally
        {
            Cursor = Cursors.Default;
