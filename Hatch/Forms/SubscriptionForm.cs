...
public partial class SubscriptionForm : Form
{
    ...
    private void SubscriptionForm_FormClosing(object sender, FormClosingEventArgs e)
    {
        Configuration.SaveAsync().ContinueWith(t =>
        {
            if (t.IsFaulted && t.Exception != null)
                Log.Error(t.Exception, "Save configuration on form closing failed");
        });
    }
    ...
    private void AddButton_Click(object sender, EventArgs e)
    {
        ...
        if (!LinkTextBox.Text.StartsWith("HTTPS://", StringComparison.OrdinalIgnoreCase))
        {
            MessageBoxX.Show(i18N.Translate("Subscription link must use HTTPS for security"));
            return;
        }
