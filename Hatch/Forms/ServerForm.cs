...
public abstract class ServerForm : Form
{
    ...
    protected void AddDivider(string text)
    {
        _controlLines++;
        var label = new Label
        {
            AutoSize = true,
            Location = new Point(10, ControlLineHeight * _controlLines + 5),
            Name = $"DividerLabel_{_controlLines}",
            Text = text,
            Font = new Font(Font, FontStyle.Bold),
            ForeColor = Color.DimGray
        };

        ConfigurationGroupBox.Controls.Add(label);

        // Add a small horizontal line below the label
        var line = new Label
        {
            AutoSize = false,
            Height = 1,
            ...
            Location = new Point(10, label.Bottom + 2)
        };
        ConfigurationGroupBox.Controls.Add(line);
        _controlLines++; // Take up an extra line for spacing
    }
